-- Fix: execute_burn_atomic silently swallows clearing/custody exceptions
-- Replace NULL with RAISE NOTICE so errors are logged to Postgres logs
    ...
    IF v_issuer = p_redeemer_group_id THEN
      ...
      IF v_has_custody THEN
        ...
        EXCEPTION WHEN OTHERS THEN
          RAISE NOTICE 'custody_release_committed failed for group=% amount=% err=%',
            v_issuer, v_issuer_balance, SQLERRM;
        END;
      END IF;
    ELSE
      ...
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'settle_clearing failed for settlement=% err=%',
          v_settlement_id, SQLERRM;
      END;
    END IF;
  END LOOP;

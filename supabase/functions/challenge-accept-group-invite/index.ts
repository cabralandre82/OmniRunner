...
/**
 * challenge-accept-group-invite — Supabase Edge Function
 *
 * Staff of the invited assessoria accepts or declines a team challenge invite.
 * When accepted, athletes from both groups can join via challenge-join.
 *
 * POST /challenge-accept-group-invite
 * Body: { invite_id: string, accept: boolean }
 */

const FN = "challenge-accept-group-invite";
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: invite, error: fetchErr } = await db
      ...
      .select("id, challenge_id, to_group_id, status")
      .eq("id", invite_id)
      .maybeSingle();
    ...
    if (!invite) {
      status = 404;
      return jsonErr(404, "NOT_FOUND", "Convite não encontrado", requestId);
    }

    if (invite.status !== "pending") {
      status = 409;
      return jsonErr(409, "ALREADY_RESPONDED", `Convite já ${invite.status === "accepted" ? "aceito" : "recusado"}`, requestId);
    }

    // Verify caller is staff of invited group
    ...
    if (!membership || !["admin_master", "coach"].includes(membership.role)) {
      status = 403;
      return jsonErr(403, "FORBIDDEN", "Apenas staff da assessoria convidada pode responder", requestId);
    }
    ...
    if (updateErr) {
      if (updateErr.code === '23505') {
        status = 409;
        return jsonErr(409, "ALREADY_ACCEPTED", "Convite já aceito", requestId);
      }
    ...
    // If declined, clear team_b_group_id so another group can be invited
    if (!accept) {
      await db
        .from("challenges")
        .update({ team_b_group_id: null })
        .eq("id", invite.challenge_id)
        .eq("team_b_group_id", invite.to_group_id);
    }

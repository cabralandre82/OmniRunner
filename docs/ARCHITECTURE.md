...
| F29 — Portal Audit Trail | portal + migration | Append-only log de ações do staff e platform admin | ✅ |
| F30 — Portal Branding | portal + migration | Logo, cores, temas por assessoria | ✅ |
| F31 — OmniCoin Distribution | portal API | Distribuição individual via portal | ✅ |
...
Next.js 14 portal (Vercel). Supabase Auth SSR. Stripe + MercadoPago (card/pix/boleto). Auto top-up com Stripe saved card ou push manual. Refund com fluxo approved→processed. Distribuição de OmniCoins por atleta via portal. Exportação CSV. Dashboard com gráficos e tendências. Branding personalizável (logo, cores, temas). Audit trail append-only. Rate limiting. RBAC com DB re-verification. Nunca no app mobile.
...
| Supabase | 55 Edge Functions + 63 migrations |
...
### Portal Assessoria — Páginas

| Rota | Descrição |
|------|-----------|
| `/dashboard` | KPIs (créditos, atletas, verificados, ativos 7d), gráfico atividade 7d, tendências week-over-week, alerta créditos baixos |
| `/credits` | Saldo de créditos, pacotes disponíveis, compra via checkout |
| `/billing` | Histórico de compras da assessoria |
...
| Rota | Descrição |
|------|-----------|
| `POST /api/checkout` | Inicia checkout via Edge Function (Stripe/MercadoPago) |
| `POST /api/billing-portal` | Redireciona para portal Stripe do cliente |
| `POST /api/auto-topup` | Configuração de recarga automática |
| `GET/POST /api/branding` | Leitura e atualização de identidade visual |
| `POST /api/distribute-coins` | Distribuição de OmniCoins para atleta individual |
| `GET /api/export/athletes` | Exportação CSV de atletas com métricas |
| `POST /api/gateway-preference` | Gateway de pagamento preferido |
| `POST /api/team/invite` | Convite de staff (RPC fn_get_user_id_by_email) |
| `POST /api/team/remove` | Remoção de staff |
| `POST /api/verification/evaluate` | Reavaliação de atleta |
| `POST /api/platform/assessorias` | Ações admin (approve/reject/suspend) |
| `POST /api/platform/refunds` | Gestão de reembolsos |
| `POST /api/platform/products` | CRUD de produtos |
| `POST /api/platform/support` | Gestão de tickets |
| `POST /api/platform/liga` | Gestão de ligas |

### Portal — Segurança & Infraestrutura

| Item | Implementação |
|------|---------------|
| Auth | SSR via Supabase Auth (cookies httpOnly, middleware) |
| RBAC | Middleware + DB re-verification em cada API route |
| Multi-tenant | `portal_group_id` cookie + RLS + DB queries filtradas |
| Rate limiting | In-memory sliding window em todas as APIs de mutação |
| Audit trail | `portal_audit_log` append-only (15+ ações logadas) |

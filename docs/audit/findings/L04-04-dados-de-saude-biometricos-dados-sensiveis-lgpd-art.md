...
note: "RPCs fn_can_read_athlete_health / fn_log_sensitive_access / fn_read_athlete_health_snapshot entregues; service_role clients no Portal refatorados para `ensureCoachHealthAccess` antes de acessar sessions/runs/baselines/trends. E2E: 165/165 migrations + 146/146 tests verdes (2026-04-17)."
---
...
- `2026-04-17` — Validação end-to-end (`tools/validate-migrations.sh`): fresh-install aplica 165/165 migrations; integration suite 146/146 verde. Duas regressões próprias corrigidas:
  - `public.runs` era assumida como sempre presente. Agora a policy `runs_coach_consent_read` e o registry `sensitive_health_columns` para `runs.*` ficam em bloco defensivo (`IF EXISTS`); invariante de políticas passou de 4 para "3 core + 1 condicional".
  - Invariante do registry relaxada de `≥ 11` para `≥ 9` (colunas de tabelas opcionais — `runs.*`, `support_tickets.description`, `coaching_athlete_kpis_daily.*`, `running_dna_profiles.profile_json` — só se registram se a coluna existir no ambiente).

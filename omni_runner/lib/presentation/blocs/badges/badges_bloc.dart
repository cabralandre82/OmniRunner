...
class BadgesBloc extends Bloc<BadgesEvent, BadgesState> {
  ...
  Future<void> _fetch(Emitter<BadgesState> emit) async {
    try {
      await _evaluateRetroactive();
      final catalog = await _fetchCatalogFromServer();
      await _syncAwardsFromServer();
  ...
  Future<void> _evaluateRetroactive() async {
    if (!AppConfig.isSupabaseReady || _userId.isEmpty) return;
    try {
      await Supabase.instance.client
          .rpc('evaluate_badges_retroactive', params: {'p_user_id': _userId});
    } catch (_) {}
  }

  Future<List<BadgeEntity>> _fetchCatalogFromServer() async {
    if (!AppConfig.isSupabaseReady) return const [];
    try {
      final rows = await Supabase.instance.client
          .from('badges')
          .select('id, category, tier, name, description, xp_reward, coins_reward, criteria_type, criteria_json, is_secret')
          .order('category')
          .order('tier');
  ...
  Future<void> _syncAwardsFromServer() async {
    if (!AppConfig.isSupabaseReady || _userId.isEmpty) return;
    try {
      final rows = await Supabase.instance.client
          .from('badge_awards')
  ...
  static BadgeCriteria _parseCriteria(String type, Map<String, dynamic> json) =>
      switch (type) {
        ...
        'daily_streak' =>
          DailyStreak((json['days'] as num?)?.toInt() ?? 0),
        'weekly_distance' =>
          SingleSessionDistance((json['threshold_m'] as num?)?.toDouble() ?? 0),
        ...
        'personal_record_pace' =>
          PersonalRecordPace(minDistanceM: (json['min_distance_m'] as num?)?.toDouble() ?? 1000),
        'single_session_duration' =>
          SingleSessionDuration((json['threshold_ms'] as num?)?.toInt() ?? 0),
        'lifetime_duration' =>

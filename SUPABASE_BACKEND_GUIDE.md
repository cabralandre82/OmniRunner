...
10. **Decisões arquiteturais**: `docs/DECISIONS_LOG.md` (123 decisões documentadas, incluindo billing, auto top-up, refunds, limites, strava-only, parks, social, push, polimento, liga, wrapped, running dna, backend audit, liga estadual, auditoria final, Strava OAuth fix, cooperative→team, FlutterWebAuth2, verificação min-distance + Strava backfill, Strava connect AuthFailed fix, CallbackActivity scheme dedicado, auto-backfill na verificação, eval RPC direto, flagged runs ignore short, eval_my_verification wrapper, session status completed=3 fix + Isar unique index fix, today screen Supabase fetch + remove Abrir Strava, TodayScreen reload ao trocar aba + isConnected resiliente, verificação reimport Strava + filtro 1km + Supabase fetch, auditoria Isar-only 7 telas corrigidas, backfill park_activities para corridas importadas, auditoria raios de parques + 9 capitais faltantes).

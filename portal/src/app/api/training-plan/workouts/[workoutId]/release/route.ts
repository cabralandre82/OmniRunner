...
const ReleaseSchema = z.object({ reason: z.string().max(200).optional() });

type Params = { params: { workoutId: string } };

/** POST /api/training-plan/workouts/[workoutId]/release — libera manualmente */
export const POST = withErrorHandler(async (req: NextRequest, { params }: Params) => {
  ...
  const { data: result, error } = await supabase.rpc("fn_release_workout", {
  ...
  if (groupId) {
    await auditLog({
      actorId: user.id,
      groupId,
      action: "workout_release.released",

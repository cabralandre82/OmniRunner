...
/**
 * L06-13 — every log line auto-carries the per-request `request_id`
 * generated by the portal middleware (L13-06). Edge Functions emit
 * the same field via `crypto.randomUUID()` plus the `x-request-id`
 * header forwarded from the portal, so a single log search across
 * portal + Edge Functions returns every line that handled the same
 * user click.
 *
 * `next/headers` `headers()` is only callable in server contexts
 * (RSC, route handler, server action); we wrap the lookup in a
 * try/catch so background jobs and tests that import `logger` from
 * a non-request scope still work.
 *
 * The lookup is intentionally lazy — `import { headers } from
 * "next/headers"` would force every consumer (including tests) to
 * be in a Next.js request context. Dynamic import keeps the surface
 * compatible with vitest.
 */
function activeRequestContext(): { request_id?: string } {
  // `next/headers` `headers()` only works inside a Next.js request
  // scope (RSC, route handler, server action, server-side
  // middleware). Outside that scope (background jobs, vitest,
  // tracing.ts top-level) the call throws — we swallow that and
  // simply omit the field rather than crash the log call.
  try {
    const id = nextHeaders().get("x-request-id");
